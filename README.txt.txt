CLASS NUMBER: CPSC 335-04 13941
PROJECT NUMBER: 3 
PROJECT NAME: 335-project-3
TEAM NAME: ACEL
TEAM MEMBERS: Alexandra Paul, Chris Ahn, Elias Jackson, Luan Truong
-------------------------------------------------------------------------------------------------

INTRODUCTION: This program searches for the designated coordinate from a starting point on a grid.
The movement follows the chess rules for the knight piece. On the grid, it can move either 2 spaces
horizontally and 1 veritcally or 1 horizontally and 2 vertically. Moreover, each grid cell shows 
its capacity, so the cells will be represented as grid boxes. This means knight movement will take place 
from the middle of each grid cell, not from the line intersections. By looking at all possible edges, and 
their capacities, from the source and sink, we can take the maximum of these capacity values and denote this 
as the maximum possible flow. Edges of the source and sink can be found by using the function 
...
Make sure to include the canvas.js in order for the grid to appear and work.
